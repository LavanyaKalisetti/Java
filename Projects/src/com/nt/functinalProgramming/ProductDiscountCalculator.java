/*
  ...
  Create a Java program that calculates the discounted price of products based on their original prices 
  and implements the Consumer functional interface using lambda expressions. 
  The program should allow the user to input the prices of multiple products 
  and then calculate and display their discounted prices.
...
public class ProductDiscountCalculator {
    public static void main(String[] args) {
        ...
        Consumer<Double> applyDiscount = price -> {
            double discounted = price * 0.90; 
            System.out.printf("Discounted price: %.2f\n", discounted);
        };

        System.out.println("\nDiscounted Prices:");
        for (double price : prices) {
            applyDiscount.accept(price);
        }
    }
}
